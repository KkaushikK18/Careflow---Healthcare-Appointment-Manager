...
// Replace DoctorDashboard
const doctorDashRegex = /function DoctorDashboard\(\{ appointments, isLoading \}: any\) \{[\s\S]*?\} \}/;

// Let's create the new DoctorDashboard
...
// Make sure RecordsView passes token and refetchLeaves to DoctorDashboard
const recordsViewRouteRegex = /if \(active === 'Overview' \|\| active === 'Today'\) return role === 'patient' \? <PatientDashboard go=\{go\} appointments=\{realAppointments\} meds=\{realMeds\} isLoading=\{appsLoading \|\| medsLoading\}\/> : role === 'doctor' \? <DoctorDashboard appointments=\{realAppointments\} isLoading=\{appsLoading\}\/> : <AdminDashboard metrics=\{adminMetrics\} isLoading=\{adminLoading\}\/>/;
